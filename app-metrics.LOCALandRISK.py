...
def tile_data():
    ...
    if auth  == pulled_token:
        ...
        # # send  data to be formatted for the tile..  
            return jsonify_data(metric_tile_data_format_events(start , low , medium , high , veryhigh))
        
        elif req['tile_id'] == 'risk-count':
            high, medium, low, total = get_risk_count() 
            return jsonify_data(vert_bar_chart_tile_data_format_risk(high, medium, low, total))
